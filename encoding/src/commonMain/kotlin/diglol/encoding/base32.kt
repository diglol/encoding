...
private fun ByteArray.encodeBase32(map: ByteArray = BASE32): String {
  ...
  while (i < end) {
    ...
    repeat(5) {
      b0 = (b0 shl 8) + this.retrieveBits(i++)
    }
  ...
  when (size - end) {
    ...
    2 -> { // 8*2 = 16 bits
      val b0 = this.retrieveBits(i++)
      val b1 = (b0 shl 8) + this.retrieveBits(i)
    ...
    3 -> { // 8*3 = 24 bits
      val b0 = this.retrieveBits(i++)
      val b1 = (b0 shl 8) + this.retrieveBits(i++)
      val b2 = (b1 shl 8) + this.retrieveBits(i)
    ...
    4 -> { // 8*4 = 32 bits
      ...
      val b1 = (b0 shl 8) + this.retrieveBits(i++)
      val b2 = (b1 shl 8) + this.retrieveBits(i++)
      val b3 = (b2 shl 8) + this.retrieveBits(i)
...
private fun String.decodeBase32(map: ByteArray = BASE32): ByteArray? {
  val isBase32 = map.contentEquals(BASE32)
...
private inline fun ByteArray.retrieveBits(index: Int): Long {
  val bits = this[index].toLong()
  return if (bits < 0) bits + 256L else bits
}

fun ByteArray.encodeBase32(): String = encodeBase32(BASE32)
fun String.decodeBase32(): ByteArray? = decodeBase32(BASE32)

fun ByteArray.encodeBase32Hex(): String = encodeBase32(BASE32_HEX)
fun String.decodeBase32Hex(): ByteArray? = decodeBase32(BASE32_HEX)
